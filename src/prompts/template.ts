...
let skipPromptsModified = (options: any, defaultFolderName: string, notAmongTemplateCollection: any, defaultTemplate: any) => {
...
type IfolderType = { folderName: string };

export const templateMissingOptionPrompt = async (
  options: Ioptions,
  folderNameAnswers: IfolderType,
  ...
  if (notAmongTemplateCollection && options?.template !== undefined && !options.skipPrompts) {
    console.log(`${chalk.redBright(`${options.template}`)} is not in the node-mongo template collection`);
  }
